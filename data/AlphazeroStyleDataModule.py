...
class AlphazeroStyleDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        if stage == "fit":
            self.train_dataset = AlphazeroCommentaryDataset(
                self.train_config,
                self.sp
            )
            self.val_dataset = AlphazeroCommentaryDataset(
                self.val_config,
                self.sp
            )
        elif stage == "test":
            self.test_dataset = AlphazeroCommentaryDataset(
                self.test_config,
                self.sp
            )
        else:
    ...
    def get_board_channels(self):
        return AlphazeroCommentaryDataset.get_board_channels(self.train_config.count_past_boards)