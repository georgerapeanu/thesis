...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, encoder_output: torch.Tensor, X: torch.Tensor, pad_mask: torch.Tensor) -> torch.Tensor:
        X = self.layer_norm1(X + self.dropout(self.mha1(query=X, key=X, value=X, need_weights=False, key_padding_mask=pad_mask, attn_mask=self.attn_mask[:X.size(1), :X.size(1)])[0], is_casual=True))
